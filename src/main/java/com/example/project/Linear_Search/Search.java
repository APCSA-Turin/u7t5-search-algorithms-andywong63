...
public class Search {
    ...
    public static int linearSearchFirst(ArrayList<Integer> elements, int target) {
        // https://stackoverflow.com/a/55172521
        return linearSearchFirst(elements.stream().mapToInt(Integer::valueOf).toArray(), target);
    }

    public static int linearSearchLast(ArrayList<Integer> elements, int target) {
        return linearSearchLast(elements.stream().mapToInt(Integer::valueOf).toArray(), target);
    }

    public static boolean linearSearchFound(ArrayList<Integer> elements, int target) {
        return linearSearchFound(elements.stream().mapToInt(Integer::valueOf).toArray(), target);
    }

    public static int linearSearchCount(ArrayList<Integer> elements, int target) {
        return linearSearchCount(elements.stream().mapToInt(Integer::valueOf).toArray(), target);
    }
}